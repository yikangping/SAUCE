...
class CensusDataset(Dataset):
    def __init__(self, split='train', frac=None):
        path = os.path.join(data_PATH, '{}_int.npy'.format(dataset_name))

        self.data = np.load(path, allow_pickle=True).astype(np.float32)

        self.n, self.dim = self.data.shape

        rng = np.random.RandomState(1234)
        noise = rng.rand(self.data.shape[0], self.data.shape[1])
