...
# sys.path.append("/home/kangping/code/LearnedCardinalityUpdater")
# sys.path.append("./")
...
COLS = {
    ...
    "BJAQ_sub": ["PM2.5", "PM10", "NO2", "O3", "TEMP"],
}
...
def LoadOracleCardinalities(dataset_name, querySeed=1234):
    ORACLE_CARD_FILES = {
        "power": PROJECT_PATH
        + "evaluate/oracle/{}_rng-{}.csv".format(dataset_name, querySeed),
        "bjaq": PROJECT_PATH
        + "evaluate/oracle/{}_rng-{}.csv".format(dataset_name, querySeed),
    }
...
deltas = {"power": np.zeros(6), "bjaq": np.array([1, 1, 1, 1, 0.1])}
...
class DataWrapper:
    ...
    def getLegalRangeQuery(self, query):
        ...
        legal_list = [[0.0, 1.0]] * len(vals)
        i = 0
        for co, op, val_i in zip(cols, ops, vals):
            col_id = self.getColID(co)
            if val_i is None:
                legal_list[i] = self.GetLegalRange(
                    co, "in", [self.Mins[col_id], self.Maxs[col_id]]
                )
            else:
    ...
    def getAndSaveOracle(self, queries, querySeed=2345):
        ...
        print(
            PROJECT_PATH
            + "evaluate/oracle/{}_rng-{}.csv".format(self.dataset_name, querySeed)
        )
        df.to_csv(
            PROJECT_PATH
            + "evaluate/oracle/{}_rng-{}.csv".format(self.dataset_name, querySeed),
