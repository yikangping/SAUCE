...
class KDLoss(nn.Module):
    ...
    def forward(self, student_logit, teacher_logit, target, loss_original):
        ...
        kl = F.kl_div(
            F.log_softmax(student_logit / self.temperature, dim=-1),
            F.softmax(teacher_logit / self.temperature, dim=-1),
            reduction="batchmean",
        )
...
class MADE(nn.Module):
    def __init__(
        ...
        if fixed_ordering is not None:
            assert num_masks == 1
            print(
                "** Fixed ordering {} supplied, ignoring natural_ordering".format(
                    fixed_ordering
                )
            )
