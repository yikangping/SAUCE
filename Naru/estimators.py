...
def QueryToPredicate(columns, operators, vals, wrap_as_string_cols=None):
    ...
    v_s = [str(v).replace("T", " ") if type(v) is np.datetime64 else v for v in vals]
    ...
    preds = [c.pg_name + " " + o + " " + v for c, o, v in zip(columns, operators, v_s)]
    s = " and ".join(preds)
    return " where " + s
            ...
            if i < ncols - 1:
                ...
                if i != 0:
                    ...
                    num_i = (
                        num_samples
                        if num_samples
                        else int(self.r * self.dom_sizes[natural_idx])
                    )
...
class SampleFromModel(CardEst):
    ...
    def SampleTuples(self, num):
        ...
        samples = self.model.sample(num, self.device).to(torch.int32).cpu().numpy()
        return samples
...
class Heuristic(CardEst):
    ...
    def Query(self, columns, operators, vals):
        ...
        sels = [
            OPS[o](c.data if isinstance(c.data, np.ndarray) else c.data.values, v).sum()
            / self.size
            for c, o, v in zip(columns, operators, vals)
        ]
        sel = np.prod(sels)
...
class BayesianNetwork(CardEst):
    ...
    def build_discrete_mapping(self, table, discretize, discretize_method):
        assert discretize_method in ["equal_size", "equal_freq"], discretize_method
    ...
    def Query(self, columns, operators, vals):
        ...
        assert ncols == self.discrete_table.shape[1], (ncols, self.discrete_table.shape)
        ...
        def draw_conditional_pgm(evidence, col_id):
            ...
            if operators[col_id] is None:
                ...
                if self.discretize:
                    ...
                    if val == 0 and operators[col_id] == "<":
                        val += 1
                    elif val == self.max_val[col_id] and operators[col_id] == ">":
                        val -= 1
            ...
            for d in xi_distribution:
                ...
                if denominator == 0:
                    prob = [1.0 for _ in prob]  # doesn't matter
                    if len(prob) == 0:
                        prob = [1.0]
                        keys = [0.0]
                prob = np.array(prob) / sum(prob)
                filtered_distributions.append((keys, prob))
            xi_samples = [np.random.choice(k, p=v) for k, v in filtered_distributions]
        ...
        def draw_conditional(evidence, col_id):
            ...
            xi_distribution = self.model.predict_proba(
                evidence, max_iterations=1, n_jobs=-1
            )
            xi_marginal = [prob_match(d[col_id].parameters[0]) for d in xi_distribution]
            filtered_distributions = []
            for d in xi_distribution:
                keys = []
                prob = []
        ...
        p_estimates = [1.0 for _ in range(self.num_samples)]
...
class MaxDiffHistogram(CardEst):
    ...
    def _build_histogram(self):
        ...
        for cid in range(len(self.table.columns)):
            if not self.table.columns[cid].data.dtype == "int64":
                ...
                p.boundaries.append(
                    (
                        min(self.table.columns[cid].data),
                        max(self.table.columns[cid].data),
                        True,
                    )
                )
        # include all rowids
        ...
        while len(self.partitions) < self.limit:
            ...
            (
                split_partition_index,
                split_column_index,
                partition_boundaries,
                global_maxdiff,
            ) = self.next_partition_candidate(
                self.partitions,
                len(self.table.columns),
                self.table,
                min(self.num_new_partitions, self.limit - len(self.partitions) + 1),
                self.maxdiff,
            )
            ...
            for p in new_partitions:
                self.partition_to_maxdiff[p] = set()
                self._compute_maxdiff(p)
            for d in self.partition_to_maxdiff[self.partitions[split_partition_index]]:
                remove_set = set()
                for cid in range(len(self.table.columns)):
                    remove_set.add((self.partitions[split_partition_index], cid))
                for tp in remove_set:
                    if tp in self.maxdiff[d]:
                        self.maxdiff[d].remove(tp)
                if len(self.maxdiff[d]) == 0:
                    del self.maxdiff[d]
            del self.partition_to_maxdiff[self.partitions[split_partition_index]]
        ...
        for pid, partition in enumerate(self.partitions):
            ...
            for cid, boundary in enumerate(partition.boundaries):
                distinct = len(
                    set(
                        self.table.columns[cid].data[rowid]
                ...
                if distinct == 1:
                    if not self.table.columns[cid].data.dtype == "int64":
                        partition.uniform_spreads.append(
                            [
                                list(
                                    set(
                                        self.table_ds.tuples_np[
                                            partition.data_points, cid
                                        ]
                                    )
                                )[0]
                            ]
                        ...
                        partition.uniform_spreads.append(
                            [
                                list(
                                    set(
                                        self.table.columns[cid].data[
                                            partition.data_points
                                        ]
                                    )
                                )[0]
                            ]
                    ...
                    spread_length = None
                    if boundary[2]:
                        spread_length = float((boundary[1] - boundary[0])) / (
                            distinct - 1
                        )
                        uniform_spread = [boundary[0]]
                    else:
                        spread_length = float((boundary[1] - boundary[0])) / (distinct)
                        uniform_spread = [boundary[0] + spread_length]
                    for _ in range(distinct - 2):
                        uniform_spread.append(uniform_spread[-1] + spread_length)
        ...
        for cid in range(len(self.table.columns)):
            for pid, partition in enumerate(self.partitions):
                if partition.boundaries[cid][0] not in self.column_bound_map[cid]["l"]:
                    self.column_bound_map[cid]["l"][partition.boundaries[cid][0]] = [
                        pid
                    ]
                else:
                    self.column_bound_map[cid]["l"][
                ...
                if partition.boundaries[cid][1] not in self.column_bound_map[cid]["u"]:
                    self.column_bound_map[cid]["u"][partition.boundaries[cid][1]] = [
                        pid
                    ]
                else:
                    self.column_bound_map[cid]["u"][
                ...
                self.column_bound_index[cid]["l"].append(partition.boundaries[cid][0])
                self.column_bound_index[cid]["u"].append(partition.boundaries[cid][1])
    ...
    def next_partition_candidate(
        self, partitions, column_number, table, num_new_partitions, maxdiff_map
    ):
        ...
        partition_boundaries = sorted(
            list(
                tp[0]
                for tp in sorted(diff, key=operator.itemgetter(1), reverse=True)[
                    : min(num_new_partitions - 1, len(diff))
                ]
            )
        )
        return (partitions.index(partition), cid, partition_boundaries, global_maxdiff)

    def generate_new_partitions(
        self, partition, partition_column_index, partition_boundaries
    ):
        new_partitions = []
        ...
        for rowid in partition.data_points:
            ...
            new_partitions[
                bisect.bisect_left(partition_boundaries, val)
            ].data_points.append(rowid)
        ...
        for new_partition in new_partitions:
            for cid in range(len(new_partition.boundaries)):
                new_partition.col_value_list[cid] = [
                    partition.col_value_list[cid][partition.rowid_to_position[rowid]]
            ...
            if len(new_partition.data_points) == 0:
                ...
                print(
                    sorted(
                        list(
                            self.table.columns[partition_column_index].data[rowid]
                            for rowid in partition.data_points
                        )
                    )
                )
    ...
    def _populate_column_set_map(self, c, o, v, column_set_map):
        ...
        if o in ["<", "<="]:
            insert_index = None
            if o == "<":
                insert_index = bisect.bisect_left(self.column_bound_index[cid]["l"], v)
            ...
            insert_index = None
            if o == ">":
                insert_index = bisect.bisect(self.column_bound_index[cid]["u"], v)
            else:
                insert_index = bisect.bisect_left(self.column_bound_index[cid]["u"], v)
            for i in range(insert_index, len(self.column_bound_index[cid]["u"])):
                column_set_map[cid] = column_set_map[cid].union(
                    self.column_bound_map[cid]["u"][
                        self.column_bound_index[cid]["u"][i]
                    ]
                )
        else:
            assert o == "=", o
    ...
    def _estimate_cardinality_per_partition(self, partition, columns, operators, vals):
