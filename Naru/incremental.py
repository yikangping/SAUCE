...
class Incremental:
    ...
    def __init__(
        ...
        if data_dir == "cn_data":
            ...
            qlib.init(
                provider_uri=root_path + data_dir,
        ...
        self.not_sequence = (
            self.forecast_model in ["MLP", "Linear"] and self.alpha == 158
        )
        self.h_path = h_path
    ...
    def dump_data(self):
        ...
        assert (
            ta.align_seg(t["dataset"]["kwargs"]["segments"]["train"])[0]
            == data.index[0][0]
        )
        ...
        if "pt_model_kwargs" in rolling_task["model"]["kwargs"] and self.alpha == 158:
            self.factor_num = rolling_task["model"]["kwargs"]["pt_model_kwargs"][
                "input_dim"
            ]
        ...
        if self.use_extra:
            rolling_task["dataset"]["kwargs"]["segments"]["extra"] = (
                extra_begin,
                extra_end,
            )
    ...
    def online_training(self, meta_tasks_test, meta_model, tag=""):
        ...
        with R.start(experiment_name=self.infer_exp_name):
            ds = init_instance_by_config(
                self.basic_task["dataset"], accept_types=Dataset
            )
            ...
            print(
                ...
                meta_model.lr_model,
                "lr_ma:",
                meta_model.framework.opt.param_groups[0]["lr"],
                "lr_da:",
                meta_model.opt.param_groups[0]["lr"],
            )
            # tasks = []
    ...
    def backtest(self, pred_y_all):
        ...
        mse = (
            (pred_y_all["pred"].to_numpy() - pred_y_all["label"].to_numpy()) ** 2
        ).mean()
        mae = np.abs(
            pred_y_all["pred"].to_numpy() - pred_y_all["label"].to_numpy()
        ).mean()
    ...
    def run_all(self):
        ...
        for i in range(0, 5):
            ...
            try:
                ...
                rec = R.get_exp(
                    experiment_name=self.meta_exp_name + "_checkpoint"
                ).list_recorders(rtype=Experiment.RT_L)[i]
